...
def crop_jewelry_image(image_path):
    ...
    if contours:
        ...
        # Add smart padding based on jewelry size
        padding = max(5, min(w, h) // 20)  # Dynamic padding based on jewelry size
        ...
        print(f"📦 Detected jewelry at: ({x}, {y}) size: {w}x{h}")
        print(f"✂️ Final crop area: ({left}, {top}) to ({right}, {bottom})")
    ...
    try:
        ...
        for (x, y, w, h) in left_ears:
            ...
            if confidence > best_confidence:
                ...
                # For left ear, position earring 1/4 from the right side of detected area
                ear_x = x + int(w * 0.75)  # 1/4 from right edge
                ear_y = y + h  # Bottom of ear for earring placement
                best_ear = (ear_x, ear_y)
                ear_side = "left"
        ...
        for (x, y, w, h) in right_ears:
            ...
            if confidence > best_confidence:
                best_confidence = confidence
                # For right ear, position earring 1/4 from the left side of detected area
                ear_x = x + int(w * 0.25)  # 1/4 from left edge
                ear_y = y + h  # Bottom of ear for earring placement
                best_ear = (ear_x, ear_y)
                ear_side = "right"
        ...
        if best_ear:
            print(f"Best ear position for earring: {best_ear} ({ear_side} ear, confidence: {best_confidence:.3f})")
